...
\begin{infobox}
\textbf{Canonical documentation:} This PDF is a printable companion. The Markdown version at \texttt{docs/usage-guide.md} is the authoritative reference and may contain additional detail. Both are verified against SPM-Kit Data Hunter v2.2.0.
\end{infobox}
...
\bigskip
\begin{infobox}
\textbf{Note:} Data Hunter is alpha software (v2.2.0). Expect improvements to classification heuristics, source coverage, and format intelligence in future releases.
\end{infobox}
...
\bigskip
\begin{tipbox}
\textbf{Tip:} Optional API tokens (\texttt{ZENODO\_TOKEN}, \texttt{FIGSHARE\_TOKEN}, \texttt{GITHUB\_TOKEN}) may improve rate limits but are not required for public discovery.
\end{tipbox}
...
\begin{center}
\texttt{native data \quad$\rightarrow$\quad method/code \quad$\rightarrow$\quad processed output \quad$\rightarrow$\quad publication}
\end{center}
...
Every record receives one of \textbf{seven utility classes}. This is the most important classification.

\begin{center}
\begin{tabularx}{\linewidth}{l>{\RaggedRight\arraybackslash}p{5cm}>{\RaggedRight\arraybackslash}X}
\toprule
\textbf{Class} & \textbf{Criteria} & \textbf{Valid Use} \\
\midrule
\texttt{benchmark\_ready} & Distinct raw + processed assets + method/code & Analysis validation candidate (human review required) \\
...
\texttt{processed\_reference\_only} & Processed output, no recoverable raw input & Numerical context, format examples \\
...
Gold, Silver, and Bronze are compact convenience labels that combine benchmark score with utility class. They are \textbf{NOT} declarations of scientific truth.
...
\subsection{Benchmark Score (0--100)}

\begin{center}
\begin{tabular}{lr}
\toprule
\textbf{Signal} & \textbf{Points} \\
\midrule
...
Images/documents only, no data & $-$15 \\
No public files & $-$30 \\
...
The configuration is frozen at creation time.
...
\bigskip
\begin{warnbox}
\textbf{Warning:} A campaign checkpoint advances only after the entire page is committed. A failed API partition is \textbf{never} marked exhausted --- it retries on the next resume.
\end{warnbox}
...
\bigskip
\begin{warnbox}
\textbf{Warning:} Unbounded downloads require \texttt{--accept-unbounded-downloads}. This is a deliberate safety acknowledgment, not a scientific filter.
\end{warnbox}
...
\begin{tipbox}
\textbf{Tip:} A campaign can be resumed from \texttt{paused} or \texttt{stopped}. Checkpoints are page-granular. A resumed campaign skips exhausted partitions and retries failed ones.
\end{tipbox}
...
\begin{itemize}[nosep]
  ...
  \item \texttt{campaigns.sqlite3} --- source of truth for campaign state.
  \item Export files are views --- regenerate anytime with \texttt{campaign export}.
  \item Both databases use WAL journal mode.
\end{itemize}
...
\begin{itemize}[nosep]
  \item \textbf{Discovery is not validation} --- results are candidates, not benchmarks.
  \item \textbf{Raw-only files are reader fixtures} --- a \texttt{.nid} file can test a parser, not an algorithm.
  \item \textbf{Processed values are references, not absolute truth} --- units, calibration, and parameters matter.
  \item \textbf{Gwyddion is a reference, not an oracle} --- version, parameters, and operation order must be recorded.
\end{itemize}

Data Hunter guarantees: structured discovery, classification, provenance linking, file reachability verification.

Data Hunter does NOT guarantee: scientific correctness certification, automated benchmark validation, ground truth declarations.

Read the full doctrine in \texttt{SCIENTIFIC\_DATA\_HUNTING\_BIBLE.md}.

\bigskip
\begin{warnbox}
\textbf{Before reusing a dataset:} (1) read its license, (2) cite the version DOI, (3) do not redistribute restricted files, (4) preserve checksums and provenance, (5) record processing parameters, (6) consult an AFM/SPM practitioner.
\end{warnbox}
...
\begin{center}
\begin{tabularx}{\linewidth}{>{\RaggedRight}p{5cm}>{\RaggedRight}X}
\toprule
\textbf{Problem} & \textbf{Solution} \\
\midrule
Campaign won't start & Check it was created (\texttt{campaign list}), verify output directory, run \texttt{doctor}. \\[4pt]
Unbounded downloads error & Pass \texttt{--accept-unbounded-downloads} to confirm. \\[4pt]
Network errors during campaign & Engine catches exceptions per partition and continues. Retries on resume. \\[4pt]
Campaign appears stuck & Check heartbeat (\texttt{campaign status}). Reduce \texttt{--rate-seconds}. \\[4pt]
Corrupted downloads & Check checksums in catalog. Delete file and re-download. \\[4pt]
